...
while True:
    ...
    if (len(lms)!=0):
        ...
        if (showBarOnScreen):
            volBar = np.interp(length, [lMin,lMax], [250,150]) #range length to volume bar
            volPer = np.interp(length, [lMin,lMax], [0, 100]) #range length to percentage
            cv.rectangle(img, (30,250), (50, 150), (0,0,255), 2)
            cv.rectangle(img, (30,int(volBar)), (50, 250), (0,0,255), cv.FILLED)
            cv.putText(img, str(int(volPer)) + "%", (20, 280), cv.FONT_HERSHEY_PLAIN, 1.5, (0,0, 255), 2)
    ...
    if(showFPS):
        ...
        fps = 1/(cTime-pTime)
        pTime = cTime
        cv.putText(img, "FPS: " + str(int(fps)), (30,40), cv.FONT_HERSHEY_PLAIN, 2, (0,255,0), 2)
